...
const CommunityFeed = () => (
  <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 md:space-y-12 pb-20 md:pb-24">
    <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden aspect-[16/9] sm:aspect-[2/1] md:aspect-[3/1] group cursor-pointer">
      ...
      <div className="absolute bottom-0 left-0 p-4 sm:p-6 md:p-8 lg:p-12">
        ...
        <h2 className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white mb-2 tracking-tight leading-tight">Electric Dreams Tour</h2>
        <p className="text-gray-300 max-w-lg text-sm sm:text-base md:text-lg line-clamp-2 sm:line-clamp-none">Experience the sound of the future with Neon Beats. Tickets selling fast for the final leg.</p>
      </div>
    </div>
    <div className="flex gap-3 sm:gap-4 md:gap-6 overflow-x-auto pb-2 scrollbar-hide px-2">
      {[...Array(6)].map((_, i) => (
        <div key={i} className="flex flex-col items-center gap-2 sm:gap-3 flex-shrink-0 cursor-pointer group">
          ...
          <span className="text-[10px] sm:text-xs font-bold text-gray-500 group-hover:text-white transition-colors tracking-wide">ARTIST_{i}</span>
        </div>
      ))}
    </div>
        ...
        <div key={post.id} className="break-inside-avoid bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl overflow-hidden hover:border-white/20 transition-all duration-300 group">
          <div className="p-3 sm:p-4 flex items-center justify-between bg-black/20">
            <div className="flex items-center gap-2 sm:gap-3">
              <img src={post.user.image} alt={post.user.name} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover ring-2 ring-white/10" />
              <div>
                <h4 className="font-bold text-xs sm:text-sm text-white group-hover:text-fuchsia-400 transition-colors cursor-pointer">{post.user.name}</h4>
